...
void LSTMLinearLayer<Dtype>::RecurrentInputBlobNames(vector<string>* names) const {
  names->resize(2);
  (*names)[0] = "h_0";
  (*names)[1] = "c_0";
}
...
template <typename Dtype>
void LSTMLinearLayer<Dtype>::RecurrentOutputBlobNames(vector<string>* names) const {
  names->resize(2);
...
template <typename Dtype>
void LSTMLinearLayer<Dtype>::OutputBlobNames(vector<string>* names) const {
  names->resize(1);
  (*names)[0] = "h";
}

template <typename Dtype>
void LSTMLinearLayer<Dtype>::FillUnrolledNet(NetParameter* net_param) const {
  ...
  for (int t = 1; t <= this->T_; ++t) {
    ...
    //         h_t := o_t .* \tanh[c_t]
    {
      ...
      lstm_unit_param->set_type("LSTMLinearUnit");
...
REGISTER_LAYER_CLASS(LSTMLinear);
